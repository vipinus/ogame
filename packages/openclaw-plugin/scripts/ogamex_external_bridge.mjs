...
async function handleAuto(rest) {
  ...
  return [
    ...
    `• optimizer: every 60s (sidecar phase 8a) — ${cur}`,
    `• expedition: every 5s (sidecar phase 8b) — template ${JSON.stringify(expCfg.template ?? DEFAULT_EXPEDITION_TEMPLATE)}`,
...
async function handleExpedition(rest) {
  ...
  const cfg = loadExpeditionConfig();
  return [
    `**expedition daemon** ✅ (always-on, sidecar phase 8b tick 5s)`,
