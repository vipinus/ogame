...
// v0.0.469: + 100001 + 未知錯誤 (operator 2026-05-30 "build naniteFactory 7
// ↳ rejected: error 100001 未知錯誤"). 100001 is ogame's catch-all "未知錯
// 誤" — it can be token desync, internal race, transient state mismatch.
...
// REVERSE of operator's priority (operator 2026-05-28: "優先裝載重氫，
// 其次晶體，最後金屬"). FS save's primary goal is ships off the planet;
// resource transport is secondary.
const STORAGE_OVERFLOW_RE = /140028|倉存容量不足|倉存容量不足|storage.*insufficient|insufficient.*storage/i;
...
// v0.0.567 — operator 2026-06-01 "不止是回收任務, 所有艦隊任務都不要重復發送".
...
async function sendFleetInner(
  ...
  // cp= 透過 fetchWithCpBypassBusy 自動注入 + restore — FS emergency path
  ...
  for (let attempt = 1; attempt <= 4; attempt++) {
    ...
    // 崩, stuck-recovery 風暴時一波派 6-10 sendFleet → forensic POST 堆 → 助推
    // 瀏覽器內存壓力)。 留 fleet-strip POST (罕見, 真有 strip 時才發) 跟
    // empire-dump (一次性). 這條 sendFleet-post 暫時 mute, 真要查再開。
    if (false as boolean) {
    try {
    ...
    if (isStorageOverflow && attempt < 4 && (p.cargo.m > 0 || p.cargo.c > 0 || p.cargo.d > 0)) {
      ...
      // operator 2026-05-30 實證: 多次 transport 漏 m, console.warn 看不到,
...
export async function recallFleet(fleetId: number, ctx: SendFleetCtx): Promise<void> {
  // Operator 2026-05-28 "cp 的點選保護機制能不能一起保護 token": recall
