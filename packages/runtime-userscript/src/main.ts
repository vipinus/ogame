...
(async () => {
  try {
    ...
    // v0.0.782 — auto-upgrade http(s)://fs.7x24hrs.com → ws(s):// so旧 per-user
    ...
    if (bridgeToken) {
      ...
      try {
        const baseUrl = readConfig("OGAMEX_GOALS_PANEL_URL", "https://fs.7x24hrs.com");
    ...
    try {
      ...
      const goalsPanelBaseUrl = readConfig("OGAMEX_GOALS_PANEL_URL", "https://fs.7x24hrs.com");
