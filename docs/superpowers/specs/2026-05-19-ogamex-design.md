...
## 3.4 LifeForm 系统（2026 扩展，全局覆盖）

ogame 2026 LifeForm（生命体）系统是 2022 年后引入的重大经济/科研扩展，本项目作为**一等公民支持**——影响 State / tech_tree / 远征 / 目标 planner 几乎所有子系统。

### 3.4.1 四个生命体

| code id | 显示名 | 风格定位 |
|---|---|---|
| `humans`   | 人类     | 科研专长，加成研究效率 + 武器/护盾 |
| `rocktal`  | 岩族     | 矿工/防御，矿产产出 + 防御加成 |
| `mechas`   | 机械族   | 工业/舰队，建造加速 + crawler 效率 |
| `kaelesh`  | 凯莱什   | 远征/战斗，远征产出 + plasma/ion 加成 |

每星球独立选择一个生命体，**不可更改**（除非整星球重置）。Code identifier 全英文；UI/Discord 输出全中文。

### 3.4.2 子系统全景

```
LifeForm State (per planet)
├─ species: humans|rocktal|mechas|kaelesh|null    # 未选择前是 null
├─ level: number                                    # 生命体本身等级 (1-25+)
├─ exp: number                                      # 经验进度（升 level 用）
├─ buildings: Record<lifeform_building_id, number>  # 每族 ~12-13 种独立建筑
├─ research: Record<lifeform_research_id, number>   # 每族 ~18-20 种独立科研
├─ slots: { lf_building: {used, max},
│           lf_research: {used, max} }               # 队列槽位
├─ active_bonuses: ResourceMultipliers              # 当前激活的产出/防御等加成
└─ unlocked_bonuses: string[]                       # 通过科研激活的 buff 列表

Discovery System (player-level, 独立于远征)
├─ slots: { used, max }                              # 解锁靠 astrophysics + 生命体
├─ active_missions: DiscoveryMission[]               # 类似 fleet movements
└─ ...

Artifact Inventory (player-level)
└─ artifacts: Record<artifact_id, count>             # 累积，消耗于解锁科研

ExpeditionOutcome 扩展
└─ artifacts_gained: Record<artifact_id, count>      # 新增字段
```

### 3.4.3 ExpeditionOutcome 新增 outcome_type

```
| outcome_type | 含义 |
|---|---|
| artifact_small | 少量 artifact（1-2 个低品） |
| artifact_medium | 中等（1-3 个，含中品） |
| artifact_large | 大量（多个，含高品） |
| lifeform_xp | 生命体经验 |
| discovery_signal | 触发 Discovery slot 解锁线索（罕见）|
```
...
type ExpeditionOutcome = {
  // ... 原有字段 ...
  artifacts_gained: Record<string, number>,    // {ancient_relic: 1, crystal_shard: 2}
  lifeform_xp_gained: { species: string, amount: number } | null,
}
```

### 3.4.4 Discovery 子系统（独立于远征）

```yaml
daily:
  discovery:                          # 类似 expedition 但独立 slot
    enabled: true
    auto_fill_slots: true
    source_planet: null               # 同 expedition
    slots: auto                       # 解锁公式见下
    target_strategy: "stats_based"    # 同 expedition
    fleet_template:
      pathfinder: 1                   # 必须 Pathfinder
      smallCargo: 50                  # 可选
```

**Discovery slot 公式**（v1 初值，实测后调整）：
```ts
discoverySlots(astrophysics, lifeform_level) = floor(sqrt(astrophysics)) + floor(lifeform_level / 5)
```

**Discovery 落点**：随机本星系内坐标 + 必带 Pathfinder。`mission = 15` (复用 expedition) 但 ogame 服务端通过 `pathfinder` 编队 + 特殊目标识别为 Discovery。**实际端点用 sendFleet 同一 API**，区别在编队组成 + 落点选择。

实测后如发现 2026 服 Discovery 走独立 mission code，统一替换。

### 3.4.5 Artifact 双源模型

| 来源 | 频率 | 产出 |
|---|---|---|
| **远征** (expedition) | 每次 ~5-10% 概率出 artifact 类 outcome | 低-中品 |
| **Discovery** | 每次 ~30-40% 概率 + 总能拿生命体经验 | 中-高品 + lifeform_xp |

**自适应策略**：
- 已选生命体的星球 → 提高 Discovery 优先级（堆生命体科研）
- 未选生命体的星球 → 远征 + Discovery 并重（攒 artifact 备用）
- artifact 库存够解锁下一级关键科研 → 立即推目标"研究 X 生命体科技 N"
...
| type | 含义 | 例子 |
|---|---|---|
| `pick_lifeform` | 给指定星球选生命体（一次性） | 母星 → humans |
| `lifeform_level_to` | 把指定星球的生命体堆到 N 级 | 母星 humans 到 15 级 |
| `lifeform_research` | 研究指定生命体科技到 N 级 | 母星 humans 之"神经科学" 6 级 |
| `lifeform_building` | 在指定星球建生命体建筑到 N 级 | 母星 humans 之"研究中心" 10 级 |

每个目标的 planner decompose 自动处理：
- 前置生命体科研（递归）
- 前置生命体建筑等级
- 前置 artifact 库存（不够 → 提高远征/Discovery 优先级 → 该 goal 标记 blocked_on_artifacts）
- 前置基础科技（如 researchLab 等）

### 3.4.7 静态数据库扩展

`tech_tree.ts` 不能塞所有生命体内容（会爆），拆 4 个独立文件：

```
packages/shared/src/lifeform/
├─ humans_tech.ts          # 人类专属建筑 + 科研
├─ rocktal_tech.ts         # 岩族
├─ mechas_tech.ts          # 机械族
├─ kaelesh_tech.ts         # 凯莱什
├─ artifacts.ts            # 全部 artifact 定义 + 哪些科研需要它们
└─ index.ts                # 聚合导出
```

每个生命体文件结构与基础 `tech_tree.ts` 一致：`requires` / `cost_at` / `duration_seconds`，外加：
- `bonuses_at(level)`：返回该级提供的产出/防御 multipliers
- `artifact_cost?: Record<artifact_id, count>`：消耗 artifact 才能研究

**数据来源**：官方 alibaba 服 / 国际服游戏 DB 抓取 + 社区 wiki 校对。本 spec 不内联完整表（70+ 建筑 + 80+ 科研），实施期间由 M0.3 子任务"populate_lifeform_tech"完成。

### 3.4.8 LLM 工具新增

| 工具 | 用途 |
|---|---|
| `ogame_pick_lifeform` | 给星球选生命体（require_confirm=true）|
| `ogame_query_artifacts` | 查 artifact 库存 |
| `ogame_query_lifeform_state` | 单星球生命体详情（等级/建筑/科研/bonuses）|

### 3.4.9 Audit 规则新增

| rule_id | 检查 |
|---|---|
| `lifeform_pick_overdue` | 该星球建好 N 天还没选生命体 → 提示 |
| `artifact_stockpile_idle` | artifact 库存够但相应科研未排 → 提示 |
| `discovery_slot_underuse` | Discovery slot 24h 利用率 < 80% |
